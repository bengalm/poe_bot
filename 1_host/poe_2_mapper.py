...
class Mapper2(PoeBotComponent):
    ...
    def filterWaystonesCanRun(self, items: List[Item], sorted=False):
        waystones_can_run = list(
            filter(lambda
                       i: i.map_tier and i.map_tier >= self.settings.min_map_tier and i.map_tier <= self.settings.max_map_tier,
                   items)
        )
        if sorted:
    ...
    def doPreparations(self):
        ...
        while True:
            if self.settings.anoint_maps and len(maps_in_inventory[0].getDeliriumMods()) == 0:
                ...
                break
            if (self.settings.waystone_upgrade_to_rare or self.settings.waystone_upgrade_to_rare_force) and \
                    maps_in_inventory[0].rarity != "Rare":
                need_to_modify_maps = True
                break
            break
        ...
        if need_to_modify_maps:
            ...
            def identifyMaps():
                ...
                unidentified_items = list(
                    filter(lambda m: m.map_tier and m.identified is False, poe_bot.ui.inventory.items))
                if len(unidentified_items) != 0:
                    while True:
            ...
            if self.settings.waystone_upgrade_to_rare or self.settings.waystone_upgrade_to_rare_force:
                ...
                maps_can_aug_regal = list(
                    filter(lambda m: m.corrupted is False and m.identified is True and m.rarity == "Magic",
                           maps_in_inventory))
                maps_can_aug = []
                maps_can_regal = []
            ...
            if self.settings.anoint_maps:
                maps_can_be_anointed = list(
                    filter(lambda m: m.corrupted is False and len(m.getDeliriumMods()) < 3, maps_in_inventory))
    ...
    def activateMap(self):
        ...
        if poe_bot.ui.map_device.place_map_window_opened is not True:
            ...
            nearest_map_objects = sorted(another_map_objects,
        ...
        if len(poe_bot.ui.map_device.place_map_window_items) != 0:
            poe_bot.raiseLongSleepException(
                "[Mapper.activateMap] len(poe_bot.ui.map_device.place_map_window_items) != 0 #TODO remove all, test below")
    ...
    def manageStashAndInventory(self, pick_consumables=False):
        ...
        if pick_consumables:
            ...
            maps_to_pick_count = random.randint(max([1, int(self.settings.keep_waystones_in_inventory * 0.6)]),
            ...
            if len(all_waystones) == 0:
                ...
                if len(all_waystones) == 0:
                    poe_bot.raiseLongSleepException(
                        "[Mapper.manageStashAndInventory] no waystones found in stash and inventory after updating")
            ...
            if len(stash_tab_indexes) != 0:
                ...
                try:
                    ...
                    print(
                        f"[Mapper.manageStashAndInventory] tab with index {stash.current_tab_index} is not in list, but we ll still check it")
                ...
                for stash_tab_index in stash_tab_indexes:
                    ...
                    if len(consumables_to_pick) != 0:
                        ...
                        for key_index in range(len(consumables_to_pick)):
                            ...
                            consumables_in_current_tab = list(
                                filter(lambda i: i.name == consumable, stash.current_tab_items))
            ...
            if len(waystones_in_inventory) == 0:
                poe_bot.raiseLongSleepException(
                    "[Mapper.manageStashAndInventory] dont have waystones after managing stash")
            ...
            if in_instance is False:
                ...
                while poe_bot.game_data.invites_panel_visible is False:
                    ...
                    try:
                        ...
                        if e.__str__() in ["area is loading on partial request",
    ...
    def exploreRoutine(self, *args, **kwargs):
        ...
        if settings.force_deli is not False and mapper.cache.delirium_mirror_activated is False:
            delirium_mirror = next(
                (
                    ...
                    if
                    e.path == "Metadata/Terrain/Gallows/Leagues/Delirium/Objects/DeliriumInitiator" and e.is_opened is not True and e.isOnPassableZone()
                ),
                None,
            )
            if delirium_mirror:
        ...
        if current_area.activators_on_map:
            activators_on_map = list(
                filter(lambda e: e.is_targetable is not False and e.path in current_area.activators_on_map,
        ...
        if settings.do_essences is not False:
            ...
            essence = next(
                (e for e in poe_bot.game_data.entities.all_entities if
                 e.is_targetable is True and ESSENCES_KEYWORD in e.path and e.isOnPassableZone()), None
            )
            if essence:
        ...
        if settings.force_breaches is not False:
            breach_entity = next(
                (e for e in poe_bot.game_data.entities.all_entities if
                 e.path == "Metadata/MiscellaneousObjects/Breach/BreachObject"), None
            )
            if breach_entity:
        ...
        if settings.do_rituals is not False:
            ritual_entity = next(
                (
                    ...
                    for e in poe_bot.game_data.entities.all_entities
                    if
                    e.path == "Metadata/Terrain/Leagues/Ritual/RitualRuneInteractable" and e.id not in mapper.cache.ritual_ignore_ids
                ),
                None,
            )
            if ritual_entity:
                ...
                corresponding_icon = next(
                    (i for i in poe_bot.game_data.minimap_icons.icons if i.id == ritual_entity.id), None)
        ...
        map_boss = next(
            (
                ...
                if e.render_name not in [
                "Volatile Core"] and e.life.health.current != 0 and e.is_hostile is not False and e.isOnPassableZone()
            ),
            None,
        )
    ...
    def onMapFinishedFunction(self):
        ...
        while time_now < map_finish_time + 1:
            ...
            if killed_someone is False and res is False:
                point = poe_bot.game_data.terrain.pointToRunAround(
                    point_to_run_around_x=point_to_run_around[0], point_to_run_around_y=point_to_run_around[1],
                    distance_to_point=15, reversed=rev
                )
        ...
        while True:
            ...
            if poe_bot.game_data.invites_panel_visible is not False:
                ...
                if random_click_iter > 15:
                    ...
                    point = poe_bot.game_data.terrain.pointToRunAround(
                        ...
                        distance_to_point=random.randint(15, 25), check_if_passable=True
                    )
                ...
                nearby_portals = list(
                    filter(lambda e: e.distance_to_player < 50, poe_bot.game_data.entities.town_portals))
                ...
                if len(nearby_portals) != 0 and time.time() > can_click_portal_after:
                    ...
                    poe_bot.refreshInstanceData()
                    nearby_portals = list(
                        filter(lambda e: e.distance_to_player < 150, poe_bot.game_data.
                               entities.town_portals))
            ...
            poe_bot.combat_module.clearLocationAroundPoint(
                {"X": self.poe_bot.game_data.player.grid_pos.x, "Y": self.poe_bot.game_data.player.grid_pos.y},
                detection_radius=50
            )
            self.exploreRoutine()
    ...
    def complete(self):
        ...
        while mapper.cache.map_completed is False:
            ...
            discovery_points = list(
                filter(lambda p: poe_bot.game_data.terrain.checkIfPointPassable(p[0], p[1]), discovery_points))
            ...
            while point_to_go is not None and mapper.cache.map_completed is False:
                ...
                if (
                        ...
                        and poe_bot.game_data.terrain.getPassableAreaDiscoveredForPercent(
                    total=True) >= mapper.settings.discovery_percent
                ):
                    ...
                    while True:
                        if len(discovery_points) == 0:
                            if mapper.settings.boss_rush is True or mapper.settings.discovery_percent > mapper.settings.default_discovery_percent:
                                print(
                                    "mapper.boss_rush is True or custom_discovery_percent and len(discovery_points) == 0")
...
class Simulacrum:
    ...
    def getTargetableAfflictionator(self):
        return next(
            (
                ...
                if
                e.is_targetable is True and e.path == "Metadata/Terrain/Gallows/Leagues/Delirium/Act1Town/Objects/DeliriumnatorAct1"
            ),
            None,
        )
    ...
    def activateWave(self):
        ...
        if afflictionator_entity:
            poe_bot.mover.goToEntitysPoint(afflictionator_entity, release_mouse_on_end=True,
...
plt.imshow(
    poe_bot.game_data.terrain.currently_passable_area[arena_center[1] - 75: arena_center[1] + 75,
...
# aug logic
maps_can_aug_regal = list(
    filter(lambda m: m.corrupted is False and m.identified is True and m.rarity == "Magic", maps_can_run))
maps_can_aug = []
maps_can_regal = []
...
while True:
    if self.settings.anoint_maps and len(maps_in_inventory[0].getDeliriumMods()) == 0:
        need_to_modify_maps = True
        break
    if (self.settings.waystone_upgrade_to_rare or self.settings.waystone_upgrade_to_rare_force) and maps_in_inventory[
        0].rarity != "Rare":
        need_to_modify_maps = True
        break
    break
...
if need_to_modify_maps:
    ...
    if self.settings.waystone_upgrade_to_rare_force:
        # aug logic
        maps_can_aug_regal = list(
            filter(lambda m: m.corrupted is False and m.identified is True and m.rarity == "Magic", maps_in_inventory))
        maps_can_aug = []
        maps_can_regal = []
