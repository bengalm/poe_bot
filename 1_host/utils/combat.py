...
from .constants import AURAS_SKILLS_TO_BUFFS, CONSTANTS, DANGER_ZONE_KEYS, FLASK_NAME_TO_BUFF, SKILL_KEYS, \
    SKILL_KEYS_WASD
...
class CombatModule:
    ...
    def killUsualEntity(self, entity: Entity, min_hp=0, max_kill_time_sec=90, is_strong=False,
    ...
    def killTillCorpseOrDisappeared(self, entity: Entity, clear_around_radius=40, max_kill_time_sec=300,
                                    step_size=random.randint(30, 35)):
        poe_bot = self.poe_bot
        ...
        if entity_to_kill.is_targetable is False or entity_to_kill.is_attackable is False:
            ...
            while True:
                ...
                if res is None:
                    break
            entity_to_kill = next((e for e in poe_bot.game_data.entities.all_entities if e.id == entity_to_kill_id),
                                  None)
    ...
    def clearLocationAroundPoint(self, point_to_run_around, detection_radius=20, till_no_enemies_around=False,
                                 ignore_keys=[]):
        """
        point_to_run_around
        {"X":1, "Y":1}
        ...
        mover.goToPoint(
            point=point_to_go, min_distance=40, release_mouse_on_end=False, custom_continue_function=build.usualRoutine,
        ...
        def enemiesAroundPoint() -> List[Entity]:
            ...
            enemies_around = list(
                filter(
                    lambda
                        e: e.grid_position.x > lower_x and e.grid_position.x < upper_x and e.grid_position.y > lower_y and e.grid_position.y < upper_y,
    ...
    def clearAreaAroundPoint(self, point, detection_radius=20, till_no_enemies_around=False, ignore_keys=[]):
        ...
        return self.clearLocationAroundPoint(
            point_to_run_around=point_dict, detection_radius=detection_radius,
            till_no_enemies_around=till_no_enemies_around, ignore_keys=ignore_keys
        )


class AutoFlasks:
    def __init__(self, poe_bot: PoeBot, hp_thresh=0.5, mana_thresh=0.5, pathfinder=False,
                 life_flask_recovers_es=False) -> None:
        self.poe_bot = poe_bot
    ...
    def useFlask(self, flask_index, flask_type="utility"):
        ...
        self.poe_bot.bot_controls.keyboard.pressAndRelease(f"DIK_{flask_index + 1}", delay=random.randint(15, 35) / 100,
    ...
    def useUtilityFlasks(self):
        ...
        sorted_flasks = sorted(poe_bot.game_data.player.utility_flasks, key=lambda f: f.index,
        ...
        for flask in sorted_flasks:
            ...
            if flask_related_buff == "flask_effect_life" or flask_related_buff == "flask_effect_mana":
                continue
            try:
                if time_now - self.flask_use_time[flask.index] < self.utility_flasks_delay or time_now - \
                        self.flask_use_time[flask.index] < 0.5:
                    continue
            except Exception:
                try:
    ...
    def useLifeFlask(self):
        ...
        if self.pathfinder is True:
            ...
            if need_to_use_flask is True:
                avaliable_life_flask = next((f for f in poe_bot.game_data.player.life_flasks if f.can_use is not False),
                                            None)
                if avaliable_life_flask is not None:
                    ...
                    self.can_use_flask_after_by_type[CONSTANTS.FLASKS.FLASK_TYPES.LIFE] = time.time() + random.randint(
                        270, 330) / 100
                    return True
                else:
                ...
                if (
                    ...
                    for flask in poe_bot.game_data.player.life_flasks:
                        if flask.can_use is True:
                            ...
                            self.can_use_flask_after_by_type[CONSTANTS.FLASKS.FLASK_TYPES.LIFE] = time.time() + (
                                        random.randint(40, 60) / 100)
                            return True
        return False
        ...
        if poe_bot.game_data.player.life.mana.getPercentage() < self.mana_thresh:
            ...
            if (
                ...
                for flask in poe_bot.game_data.player.mana_flasks:
                    ...
                    self.can_use_flask_after_by_type[CONSTANTS.FLASKS.FLASK_TYPES.MANA] = time.time() + (
                                random.randint(40, 60) / 100)
                    return True
    ...
    def activateAurasIfNeeded(self):
                ...
                if skill_name in aura_keys:
                    is_blessing = next(
                        (
                            ...
                            if
                        "SkillIsBlessingSkill" in sd.keys() or "SupportGuardiansBlessingAuraOnlyEnabledWhileSupportMinionIsSummoned" in sd.keys()
                        ),
                        None,
                    )
                    if is_blessing:
                        self.blessing_skill = BlessingSkill(poe_bot=self.poe_bot, skill_index=skill_index,
                                                            skill_name=skill_name, display_name=skill_name)
        ...
        if auras_to_activate:
            indexes_to_activate = list(
                map(lambda skill: self.poe_bot.game_data.skills.internal_names.index(skill), auras_to_activate))
...
class Skill:
    ...
    def checkIfCanUse(self):
        if self.min_mana_to_use != 0 and self.poe_bot.game_data.player.life.mana.current < self.min_mana_to_use:
            print(
                f"[Skill] cant use skill {self.display_name} cos self.poe_bot.game_data.player.life.mana.current < self.min_mana_to_use")
            return False
    ...
    def moveThenUse(self, grid_pos_x=0, grid_pos_y=0, updated_entity: Entity = None, wait_for_execution=True,
                    force=False, use_first=False):
        # hover if needed
...
class AreaSkill(Skill):
    ...
    def use(self, pos_x=0, pos_y=0, updated_entity: Entity = None, wait_for_execution=True, force=False):
        ...
        if (
                dist(
                    [self.poe_bot.game_data.player.grid_pos.x, self.poe_bot.game_data.player.grid_pos.y],
...
class SkillWithDelay(Skill):
    ...
    def update(self):
        ...
        if self.can_use_earlier is not False:
            ...
            _rv = [1, 0]
        self.can_be_used_after = self.last_use_time + self.min_delay + random.choice(
            _rv) * self.delay_random * self.min_delay
        print(
            f"[SkillWithDelay {self.display_name}]  can be used after {self.can_be_used_after} {self.last_use_time} {self.min_delay}")
        ...
        if self.minion_path_key is None:
            ...
            return len(
                list(
                    filter(
                        lambda
                            e: e.life.health.current != 0 and not e.is_hostile and e.distance_to_player < radius and self.minion_path_key in e.path,
...
class MovementSkill(Skill):
    def __init__(
            self, poe_bot: PoeBot, skill_index: int, skill_name="", display_name="MovementSkill",
            min_delay=random.randint(30, 40) / 10, can_extend_path=True
    ) -> None:
        self.min_delay = min_delay
        self.last_use_time = 0
        self.jump_multi = 2
    ...
    def use(self, pos_x=0, pos_y=0, updated_entity: Entity = None, wait_for_execution=True, force=False,
        ...
        if pos_x != 0 or updated_entity is not None:
            ...
            distance_to_next_step = dist(
                (self.poe_bot.game_data.player.grid_pos.x, self.poe_bot.game_data.player.grid_pos.y), (x, y))
...
class MovementSkill_new(SkillWithDelay):
    ...
    def use(self, pos_x=0, pos_y=0, updated_entity: Entity = None, wait_for_execution=True, force=False,
            extend_path=True, use_as_movement_skill=True):
        if self.canUse(force) is not True:
            return False
...
class Aura(Skill):
    def __init__(self, poe_bot: PoeBot, bind_key=None, use_function=None, use_delay=4, skill_type=1,
                 name="unnamed_skill", mana_cost=0) -> None:
...
class Build:
    ...
    def staticDefence(self):
        ...
        danger_zones = list(
            filter(
                lambda e: e.distance_to_player < detection_range and any(
                    list(map(lambda key: key in e.path, DANGER_ZONE_KEYS))),
        ...
        if len(danger_zones) != 0:
            ...
            if self.chaos_immune is False and any(
                    list(map(lambda s: "/LeagueArchnemesis/ToxicVolatile" in s, danger_zone_str))):
                ...
                caustic_orbs = list(filter(lambda e: "/LeagueArchnemesis/ToxicVolatile" in e.path,
                                           poe_bot.game_data.entities.all_entities))
                ...
                pass
                    ...
                    list(map(lambda s: "Metadata/Monsters/LeagueArchnemesis/LivingCrystal" in s, danger_zone_str))):
                print("dodging living crystals")
                living_crystals = list(
                    filter(lambda
                               e: "Metadata/Monsters/LeagueArchnemesis/LivingCrystal" in e.path and e.distance_to_player < 20,
                           danger_zones)
                )
...
class GenericHitter(Build):
    ...
    def __init__(self, poe_bot: PoeBot) -> None:
        ...
        main_attacking_skill = next(
            (s for s in self.poe_bot.game_data.skills.internal_names if s in GENERIC_BUILD_ATTACKING_SKILLS), None)
        if main_attacking_skill is not None:
            ...
            if attacking_buff:
                ...
                self.attacking_buff = SkillWithDelay(
                    poe_bot=poe_bot, skill_index=skill_index, min_delay=random.randint(30, 50) / 10,
                    display_name=attacking_buff
                )
        else:
        ...
        if main_attacking_skill is not None:
            ...
            self.attacking_skill = SkillWithDelay(
                poe_bot=poe_bot, skill_index=skill_index, min_delay=random.randint(1, 5) / 100,
                display_name=main_attacking_skill, min_mana_to_use=0
            )
        else:
        ...
        if blood_rage is not None:
            ...
            self.blood_rage = SkillWithDelay(poe_bot=poe_bot, skill_index=skill_index,
                                             min_delay=random.randint(30, 50) / 10, display_name=blood_rage)
        ...
        for skill_index in range(len(self.poe_bot.game_data.skills.internal_names)):
            ...
            if skill_name == "blood_rage":
                self.blood_rage = SkillWithDelay(poe_bot=poe_bot, skill_index=skill_index,
                                                 min_delay=random.randint(30, 50) / 10, display_name=skill_name)
            elif skill_name in NON_INSTANT_MOVEMENT_SKILLS:
                self.movement_skill = MovementSkill(poe_bot=poe_bot, skill_index=skill_index, display_name=skill_name,
                                                    min_delay=random.randint(30, 50) / 100)
                if skill_name == "whirling_blades":
                    self.instant_movement_skill = MovementSkill(
                        poe_bot=poe_bot, skill_index=skill_index, display_name=skill_name,
                        min_delay=random.randint(30, 50) / 100
                    )
    ...
    def usualRoutine(self, mover: Mover = None):
        ...
        if mover is not None:
            ...
            nearby_enemies = list(filter(lambda e: e.isInRoi() and e.distance_to_player < 30,
                                         poe_bot.game_data.entities.attackable_entities))
                ...
                for iiii in range(1):
                    ...
                    if self.attacking_skill.last_use_time + random.uniform(1.0, 1.5) < time_now:
                        ...
                        enemies_in_sector = list(
                            filter(lambda e: getAngle(p0, p1, (e.grid_position.x, e.grid_position.y),
                                                      abs_180=True) < search_angle_half, nearby_visible_enemies)
                        )
            ...
            if entities_to_hold_skill_on:
                ...
                while time.time() - hold_duration < hold_start_time:
                    poe_bot.refreshInstanceData()
                    self.auto_flasks.useFlasks()
                    entity_to_kill = next((e for e in poe_bot.game_data.entities.attackable_entities if
                                           e.id in entities_to_hold_skill_on_ids), None)
                    if entity_to_kill:
            ...
            if self.movement_skill and mover.distance_to_target > 50:
                ...
                if self.instant_movement_skill:
                    instant_movement_used = self.instant_movement_skill.use(mover.grid_pos_to_step_x,
                                                                            mover.grid_pos_to_step_y,
                                                                            wait_for_execution=False)
                    if instant_movement_used:
                        return True
                if self.movement_skill.use(mover.grid_pos_to_step_x, mover.grid_pos_to_step_y,
                                           wait_for_execution=False) is True:
                    return True
    ...
    def killUsual(self, entity: Entity, is_strong=False, max_kill_time_sec=random.randint(200, 300) / 10, *args,
                  **kwargs):
        ...
        entity_to_kill = next((e for e in poe_bot.game_data.entities.attackable_entities if e.id == entity_to_kill_id),
                              None)
        if not entity_to_kill:
            print("cannot find desired entity to kill")
            return True
        ...
        while True:
            ...
            self.auto_flasks.useFlasks()
            entity_to_kill = next(
                (e for e in poe_bot.game_data.entities.attackable_entities if e.id == entity_to_kill_id), None)
            if not entity_to_kill:
                print("cannot find desired entity to kill")
                break
            print(f"entity_to_kill {entity_to_kill}")
...
class DirctMinions(SkillWithDelay):
    def __init__(self, poe_bot: PoeBot):
        super().__init__(
            poe_bot=poe_bot,
            skill_index=3,
            skill_name="",
            display_name="DirctMinions",
            min_delay=0.1,
            delay_random=0.1,
            min_mana_to_use=0,
            can_use_earlier=True,
        )
        self.skill_key = "DIK_GRAVE"
        self.tap_func = poe_bot.bot_controls.keyboard.pressAndRelease
        self.press_func = poe_bot.bot_controls.keyboard_pressKey
        self.release_func = poe_bot.bot_controls.keyboard_releaseKey
        self.checkIfCanUse = lambda *args, **kwargs: True
        self.getCastTime = lambda *args, **kwargs: 1
...
class InfernalistZoomancer(Build):
    ...
    def __init__(self, poe_bot: PoeBot, can_kite=True) -> None:
        ...
        for skill_index in range(len(self.poe_bot.game_data.skills.internal_names)):
            ...
            skill_base_cast_time = next(
                (list(sd.values())[0] for sd in poe_bot.game_data.skills.descriptions[skill_index] if
                 "BaseSpellCastTimeMs" in sd.keys()), None
            )
            if skill_base_cast_time is None:
                ...
                self.minion_arconist_dd = SkillWithDelay(
                    poe_bot=poe_bot, skill_index=skill_index, min_delay=random.randint(20, 30) / 10,
                    display_name="minion_arconist_dd", can_use_earlier=False
                )
                ...
                self.minion_reaver_enrage = SkillWithDelay(
                    poe_bot=poe_bot, skill_index=skill_index, min_delay=random.uniform(2.5, 3.0),
                    display_name="minion_reaver_enrage", can_use_earlier=False
                )
        ...
        if flame_wall_index is not False:
            self.flame_wall = SkillWithDelay(
                poe_bot=poe_bot, skill_index=flame_wall_index, min_delay=random.randint(20, 30) / 10,
                display_name="flame_wall", can_use_earlier=False
            )
        ...
        if unearth_index is not False:
            self.unearth = SkillWithDelay(
                poe_bot=poe_bot, skill_index=unearth_index, min_delay=random.randint(20, 30) / 10,
                display_name="unearth", can_use_earlier=False
            )

        self.detonate_dead = None
        if detonate_dead_index is not False:
            self.detonate_dead = SkillWithDelay(
                poe_bot=poe_bot, skill_index=detonate_dead_index, min_delay=random.uniform(3.1, 4.5),
                display_name="detonate_dead", can_use_earlier=False
            )
        ...
        if offerening_index is not False:
            self.offering = SkillWithDelay(
                poe_bot=poe_bot, skill_index=offerening_index, min_delay=random.randint(20, 30) / 10,
                display_name="offering", can_use_earlier=False
            )

        self.flammability = None
        if flammability_index is not False:
            self.flammability = SkillWithDelay(
                poe_bot=poe_bot, skill_index=flammability_index, min_delay=random.randint(20, 30) / 10,
                display_name="flammability", can_use_earlier=False
            )
    ...
    def usualRoutine(self, mover: Mover = None):
        ...
        if mover is not None:
            ...
            nearby_enemies = list(filter(lambda e: e.distance_to_player < 50 and e.isInRoi(),
                                         poe_bot.game_data.entities.attackable_entities))
            print(f"nearby_enemies: {nearby_enemies}")
            ...
            if enemy_to_attack is not None:
                for _i in range(1):
                    ...
                    if self.detonate_dead and self.detonate_dead.canUse():
                        ...
                        if len(corpses_around) != 0:
                            ...
                            if corpses_around[0].attack_value != 0:
                                if self.detonate_dead.use(updated_entity=corpses_around[0],
                                                          wait_for_execution=False) is not False:
                                    attack_skill_used = True
                                    break
                    if self.minion_sniper_gas_arrow and self.minion_sniper_gas_arrow.canUse():
                        if self.minion_sniper_gas_arrow.use(updated_entity=enemy_to_attack,
                                                            wait_for_execution=False) is True:
                            attack_skill_used = True
                            break
                    ...
                    if self.unearth and self.unearth.canUse():
                        ...
                        if len(corpses_around) != 0:
                            corpses_around.sort(key=lambda e: e.calculateValueForAttack())
                            if corpses_around[0].attack_value != 0:
                                if self.unearth.use(updated_entity=corpses_around[0],
                                                    wait_for_execution=False) is not False:
                                    attack_skill_used = True
                                    break
                    if self.offering and self.offering.canUse():
                        offering_spikes = list(
                            filter(lambda e: "Metadata/Monsters/OfferingSpike/PainOfferingSpike" in e.path,
                                   poe_bot.game_data.entities.all_entities)
                        )
                        if len(offering_spikes) == 0:
                            alive_skeletons_nearby = list(
                                filter(
                                    lambda
                                        e: e.is_hostile is False and "Metadata/Monsters/Skeletons/PlayerSummoned/Skeleton",
                                    poe_bot.game_data.entities.all_entities
                                )
                            )
                            if len(alive_skeletons_nearby) != 0:
                                if self.offering.use(updated_entity=alive_skeletons_nearby[0],
                                                     wait_for_execution=False) is not False:
                                    attack_skill_used = True
                                    break
                p0 = (mover.grid_pos_to_step_x, mover.grid_pos_to_step_y)
                ...
                enemies_on_way = list(
                    filter(
                        lambda e: e.distance_to_player < 15 and getAngle(p0, p1, (e.grid_position.x, e.grid_position.y),
                                                                         abs_180=True) < 45, really_close_enemies
                    )
                )
    ...
    def killUsual(self, entity: Entity, is_strong=False, max_kill_time_sec=random.randint(200, 300) / 10, *args,
        ...
        while True:
            ...
            if skill_used is False and self.detonate_dead and self.detonate_dead.canUse():
                corpses_around = poe_bot.game_data.entities.getCorpsesArountPoint(entity_to_kill.grid_position.x,
                                                                                  entity_to_kill.grid_position.y, 40)
                corpses_around = list(filter(lambda e: e.isInLineOfSight() is not False, corpses_around))
                if len(corpses_around) != 0:
                    corpses_around.sort(key=lambda e: e.calculateValueForAttack())
                    if corpses_around[0].attack_value != 0:
            ...
            if skill_used is False and self.unearth and self.unearth.canUse():
                corpses_around = poe_bot.game_data.entities.getCorpsesArountPoint(entity_to_kill.grid_position.x,
                                                                                  entity_to_kill.grid_position.y, 20)
                corpses_around = list(filter(lambda e: e.isInLineOfSight() is not False, corpses_around))
                if len(corpses_around) != 0:
                    corpses_around.sort(key=lambda e: e.calculateValueForAttack())
                    if corpses_around[0].attack_value != 0:
            ...
            if skill_used is False and self.minion_sniper_gas_arrow and self.minion_sniper_gas_arrow.canUse():
                if self.minion_sniper_gas_arrow.use(updated_entity=entity_to_kill,
                                                    wait_for_execution=False) is not False:
                    skill_used = True
...
class PathfinderPoisonConc2(Build):
    ...
    def __init__(self, poe_bot: Poe2Bot) -> None:
        ...
        pconc_on_panel = next((s for s in self.poe_bot.game_data.skills.internal_names if s == pconc_internal_name),
                              None)
        if pconc_on_panel is not None:
            ...
            self.pconc = SkillWithDelay(
                poe_bot=poe_bot, skill_index=pconc_index, min_delay=random.randint(1, 5) / 100,
                display_name=pconc_internal_name, min_mana_to_use=0
            )
            self.pconc.sleep_multiplier = 0.2

        curse_on_panel = next((s for s in self.poe_bot.game_data.skills.internal_names if s in curse_internal_names),
                              None)
        if curse_on_panel is not None:
            curse_index = self.poe_bot.game_data.skills.internal_names.index(curse_on_panel)
            self.curse = SkillWithDelay(poe_bot=poe_bot, skill_index=curse_index, min_delay=random.randint(30, 50) / 10,
                                        display_name=curse_on_panel)

        wither_totem_on_panel = next(
            (s for s in self.poe_bot.game_data.skills.internal_names if s == wither_totem_internal_name), None)
        if wither_totem_on_panel is not None:
            ...
            self.wither_totem = SkillWithDelay(
                poe_bot=poe_bot, skill_index=wither_totem_index, min_delay=random.randint(30, 50) / 10,
                display_name=wither_totem_on_panel
            )
        self.dodge_roll = DodgeRoll(poe_bot=poe_bot)
        super().__init__(poe_bot)
        self.auto_flasks = AutoFlasks(poe_bot=poe_bot)
    ...
    def usualRoutine(self, mover: Mover = None):
        ...
        if mover is not None:
            ...
            _t = time.time()
            if self.dodge_roll.last_use_time + 0.35 > _t or self.pconc.last_use_time + (
                    self.pconc.getCastTime() / 2) > _t:
            ...
            if need_dodge is not True and throw_pconc_at is None and len(nearby_enemies) != 0:
                ...
                for _i in range(1):
                    really_close_enemies = list(
                        filter(lambda e: e.distance_to_player < really_close_enemies_distance, nearby_enemies))
                    ...
                    really_close_enemies = list(
                        filter(lambda e: e.distance_to_player < really_close_enemies_distance, enemies_for_clear))
                    ...
                    if True:
                        ...
                        # else calculate val for explosion and throw, attack value > 2
                            ...
                            enemies_in_sector = list(
                                filter(lambda e: getAngle(p0, p1, (e.grid_position.x, e.grid_position.y),
                                                          abs_180=True) < search_angle_half, enemies_for_clear)
                            )
            ...
            if throw_pconc_at:
                if self.pconc.canUse() and self.pconc.use(pos_x=throw_pconc_at[0], pos_y=throw_pconc_at[1],
                                                          wait_for_execution=True) is True:
    ...
    def killUsual(self, entity: Entity, is_strong=False, max_kill_time_sec=random.randint(200, 300) / 10, *args,
        ...
        while True:
            ...
            if skill_used is False and self.pconc and self.pconc.last_use_time + (
                    self.pconc.getCastTime() / 2) < time.time():
...
class GenericBuild2(Build):
    def __init__(self, poe_bot):
        ...
        if main_attacking_skill is None:
            self.poe_bot.raiseLongSleepException(
                f"[GenericBuild2.init] couldnt find skills from {GENERIC_BUILD_ATTACKING_SKILLS}")
    ...
    def usualRoutine(self, mover: Mover):
        ...
        self.useBuffs()
        nearby_enemies = list(
            filter(lambda e: e.isInRoi() and e.isInLineOfSight(), poe_bot.game_data.entities.attackable_entities))
        if len(nearby_enemies) == 0:
            return False
...
class GenericBuild2Cautious(GenericBuild2):
    def usualRoutine(self, mover: Mover):
        ...
        _t = time.time()
        if self.dodge_roll.last_use_time + 0.35 > _t or self.attacking_skill.last_use_time + (
                self.attacking_skill.getCastTime() / 2) > _t:
            print("probably casting smth atm")
            can_do_action = False
        if can_do_action:
            self.useBuffs()
...
class LoopController:
    ...
    def start(self):
        ...
        while self.running:
            ...
            active_flask_effects_count = len(
                list(filter(lambda e: e == "flask_effect_life", poe_bot.game_data.player.buffs)))
...
class BarrierInvocationInfernalist(Build):
    def __init__(self, poe_bot):
        ...
        if barrier_button is None:
            poe_bot.raiseLongSleepException(
                "barrier set to mouse, couldnt find barrier on keyboard qwert, press it on qwert")
    ...
    def usualRoutine(self, mover: Mover):
        ...
        if self.isInDemonForm() is True and mover.distance_to_target > 50:
            ...
            distance_to_next_step = dist((poe_bot.game_data.player.grid_pos.x, poe_bot.game_data.player.grid_pos.y),
            ...
            if distance_to_next_step > 20:
                path_values = createLineIteratorWithValues(
                    (poe_bot.game_data.player.grid_pos.x, poe_bot.game_data.player.grid_pos.y),
                    (pos_x_to_go, pos_y_to_go), poe_bot.game_data.terrain.passable
                )
...
class TempestFlurryBuild(Build):
    def __init__(self, poe_bot):
        ...
        self.tempest_flurry_button_holder: ButtonHolder = ButtonHolder(self.poe_bot, flurry_button,
                                                                       max_hold_duration=0.33)
    ...
    def usualRoutine(self, mover: Mover):
        ...
        hold_flurry = False
        nearby_enemy = next(
            (e for e in poe_bot.game_data.entities.attackable_entities if e.isInRoi() and e.isInLineOfSight()), None)
        while True:
            if nearby_enemy:
                hold_flurry = True
                break
...
class InfernalistMinion(Build):
    ...
    def __init__(self, poe_bot: PoeBot) -> None:
        ...
        for skill_index in range(len(self.poe_bot.game_data.skills.internal_names)):
            ...
            skill_base_cast_time = next(
                (list(sd.values())[0] for sd in poe_bot.game_data.skills.descriptions[skill_index] if
                 "BaseSpellCastTimeMs" in sd.keys()),
                None,
            )
            if skill_base_cast_time is None:
                continue
        ...
        self.dodge_roll = DodgeRoll(poe_bot=poe_bot)
        self.dirct_minions = DirctMinions(poe_bot=poe_bot)
    ...
    def usualRoutine(self, mover: Mover = None):
        ...
        if mover is not None:
            ...
            corpses = poe_bot.game_data.entities.getCorpsesArountPoint(poe_bot.game_data.player.grid_pos.x,
                                                                       poe_bot.game_data.player.grid_pos.y, 40)
            if corpses:
            ...
            if entity_to_explode and self.detonate_dead and self.detonate_dead.canUse() and self.detonate_dead.last_use_time + attacking_skill_delay < time.time():
            ...
            corpses = poe_bot.game_data.entities.getCorpsesArountPoint(poe_bot.game_data.player.grid_pos.x,
                                                                       poe_bot.game_data.player.grid_pos.y, 35)
            if corpses:
                p0 = (mover.grid_pos_to_step_x, mover.grid_pos_to_step_y)
                p1 = (poe_bot.game_data.player.grid_pos.x, poe_bot.game_data.player.grid_pos.y)
                corpses = list(
            ...
            if entity_to_unearth and self.unearth and self.unearth.canUse() and self.unearth.last_use_time + attacking_skill_delay + 1 < time.time():
            ...
            enemies_on_way = list(
                filter(
                    lambda e: e.distance_to_player < 45 and getAngle(p0, p1, (e.grid_position.x, e.grid_position.y),
                                                                     abs_180=True) < 35,
                    nearby_enemies,
                )
            )
        ...
        while True:
            ...
            if skill_used is False and self.minion_arconist_dd and self.minion_arconist_dd.canUse():
                if self.minion_arconist_dd.use(updated_entity=entity_to_kill, wait_for_execution=False) is True:
                    skill_used = True
            if skill_used is False and self.dirct_minions and self.dirct_minions.canUse():
                if self.dirct_minions.use(updated_entity=entity_to_kill, wait_for_execution=False) is True:
                    skill_used = True
                    print('using dirct_minions------- ')
